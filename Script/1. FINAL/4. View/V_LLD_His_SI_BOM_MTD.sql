...
(
	select
		...
		[Product Type] = s.[Type]
	from LLD_FC_SI_MTD_HIS H (NOLOCK)
	left join (SELECT DISTINCT [SUB GROUP/ Brand],[Type] FROM fnc_SubGroupMaster('LLD','full')) s on  s.[SUB GROUP/ Brand] = h.SubGrp
	where HIS_QTY_BOM>0
	group by
		[SubGrp],
		--[Year],
