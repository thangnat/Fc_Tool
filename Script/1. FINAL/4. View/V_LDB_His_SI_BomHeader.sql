/*
	select * 
	from V_LDB_His_SI_BomHeader
	where Barcode = '8935274632881' and [Pass Column Header] = 'Y-2 (u) M4'

	select * from FC_BFL_Master_LDB where [SKU Code]='MB365100'
*/


Alter View V_LDB_His_SI_BomHeader
with encryption
As
select
	[Product Type],
...
(
	...
	from LDB_FC_SI_HIS_BOM_HEADER H (NOLOCK)
) as x
group by
	[Product Type],
	[SUB GROUP/ Brand],
