/*
	exec sp_get_fc_fm_original_WF_NEW_DEV2 'CPD','202407','partial'
*/
Alter proc sp_get_fc_fm_original_WF_NEW_DEV2
	@Division		nvarchar(3),
	@FM_KEY			nvarchar(6),
	...
	begin
		...
		begin
			...
			begin
				...
				begin
					if host_name() IN('VNCORPVNWKS0850')
					begin
						if @GroupColumn='SLOB'
						begin
							select @columnName=@columnName+',[SLOB VALUE]=CAST(CASE WHEN [SLOB]>0 THEN F.[SLOB]*slv.[AVERAGE] ELSE 0 END AS NUMERIC(18,0))'
						end
					end
		...
		begin
			...
			'+
			case 
				when host_name()='VNCORPVNWKS0850' then 
				'left join
				(
					select 
