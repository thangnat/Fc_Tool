/*
	...
	exec sp_Sum_FC_FM_baseLine_new_2 'CPD','202411',0,@b_Success OUT,@c_errmsg OUT
...
Alter proc sp_Sum_FC_FM_baseLine_new_2
	@Division		nvarchar(3),
	@FM_KEY			nvarchar(6),
	@AllowTotal		int,
...
BEGIN TRY
	...
	SELECT	
		...
		@n_err=0,
		@c_errmsg='', 
		@sp_name = 'sp_Sum_FC_FM_baseLine_new_2',
		@USERS = SUSER_NAME(),
		@MODIFILED = GETDATE()
	...
	if @debug>0
	begin
		select 'get data FM had imported to create standard data'
	end
	if @n_continue = 1
	begin
	...
	if @debug>0
	begin
		select 'tien hanh update FM single to wf'
	end
	IF @n_continue = 1
	BEGIN
	...
	if @debug>0
	begin
		select 'lay unit fc tao table value'
	end
	if @n_continue = 1
	begin
	...
	if @debug>0
	begin
		select 'update table value to wf fc'
	end
	IF @n_continue = 1
	BEGIN
	...
	if @debug>0
	begin
		select 'Process Bom promo to wf for online'
	end
	if @n_continue=1
	begin
		exec sp_tag_add_FC_SI_Group_FC_SI_Promo_Bom_New @Division,@FM_KEY,'ONLINE',0,@b_Success OUT,@c_errmsg OUT
