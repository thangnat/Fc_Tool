/*
	...
	exec sp_Gen_BomHeader_Forecast_New2 'LDB','202409','','ALL',@b_Success OUT,@c_errmsg OUT
...
*/
Alter Proc sp_Gen_BomHeader_Forecast_New2
	@Division		nvarchar(3),
	@FM_Key			nvarchar(6),
	@Type			nvarchar(10),
...
BEGIN TRY
	...
	SELECT	
		...
		@n_err=0,
		@c_errmsg='', 
		@sp_name = 'sp_Gen_BomHeader_Forecast_New2',
		@USERS = SUSER_NAME(),
		@MODIFILED = GETDATE()
	...
	if @debug>0
	begin
		select 'COMBINE 2 TABLE: 1. BOM FM ONLINE, 2. BOM TEMPLATE EXCEL FILE'
	end
	if @n_continue = 1
	begin		
		if @Type=''
		begin
			exec sp_Create_List_Bom_Header_Forecast_All_New @Division,@FM_Key,@Channel,@b_Success OUT, @c_errmsg OUT
	...
	declare @channel_ok nvarchar(50)=''
	select @channel_ok=case when @channel_ok='ALL' then 'ONLINE,OFFLINE' else @channel_ok end
	if @debug>0
	begin
		select 'Delete table bom header fc'
	end
	if @n_continue = 1
	begin
		--create table initial -->Bom header fc tmp
		if @Type = ''
		begin
			if exists
			(
			...
			begin
				...
				select @sql = 'delete '+@table_FC_BomHeader+' where Channel IN(select value from string_split('''+@channel_ok+''','',''))'

				IF @debug>0
				BEGIN
					SELECT @sql 'Delete table bom header by channel'
				END
				execute(@sql)
	...
	if @debug>0
	begin
		select 'drop table bom header fc tmp'
	end
	if @n_continue=1
	begin
		if @Type=''
		begin
			...
			begin
				...
				IF @debug>0
				BEGIN
					SELECT @sql 'Drop table bom header tmp'
				END
				execute(@sql)
	...
	if @debug>0
	begin
		select 'insert bom header fc tmp'
	end
	if @n_continue=1
	begin
		if @Type = ''
	...
	if @debug>0
	begin
		select 'update bom header fc tmp'
	end
	if @n_continue=1
	begin
		if @Type = ''
		begin
			...
			where b.channel in(select value from string_split('''+@channel_ok+''','',''))'
	...
	begin
		...
		begin
			...
			begin
				select @sql = 
				'INSERT INTO '+@table_FC_BomHeader+'
				select
					*
				from '+@table_FC_BomHeader+'_Tmp b 
				where
					b.[Channel] IN(select value from string_split('''+@channel_ok+''','',''))
				and ('+@listcolum_cong_pass+'+'+@listcolumn_current_plus+')<>0 '
			end
			if @debug>0
			begin
	...
	begin
		...
		begin
			...
			set '+@ListColumn_Pass_Update+'
			from '+@table_FC_FDR_FOC_TO_VP+'_Tmp b inner join 
			(
				select 
					*
