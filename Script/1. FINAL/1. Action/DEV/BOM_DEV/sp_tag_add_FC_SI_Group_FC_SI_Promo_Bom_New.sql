/*
	...
	exec sp_tag_add_FC_SI_Group_FC_SI_Promo_Bom_New 'CPD','202410','ONLINE',0,@b_Success OUT,@c_errmsg OUT
...
Alter Proc sp_tag_add_FC_SI_Group_FC_SI_Promo_Bom_New
	@Division				nvarchar(3),
	@FM_KEY					nvarchar(6),
	@Channel				nvarchar(50),
	@AllowTotal				int,
...
BEGIN TRY
	...
	SELECT	
		...
		@n_err=0,
		@c_errmsg='', 
		@sp_name = 'sp_tag_add_FC_SI_Group_FC_SI_Promo_Bom_New',
		@USERS = SUSER_NAME(),
		@MODIFILED = GETDATE()
	...
	if @n_continue = 1
	begiN
		exec sp_Gen_BomHeader_Forecast_New2 @Division,@FM_KEY,'',@Channel,@b_Success1 OUT,@c_errmsg1 OUT
		if @b_Success1 = 0
		begin
			select @n_continue = 3, @c_errmsg = @c_errmsg1
		end
