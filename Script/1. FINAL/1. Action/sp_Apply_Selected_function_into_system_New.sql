/*
	...
	exec sp_Apply_Selected_function_into_system_New 
		'CPD',
		'202407',
		'2748,2532,2591,2593,2594,2613,2610,2527,2528,2529,2793,2614,2714',
		'NULL,P2*P4*P5,NULL,P2*P4*P5,NULL,P2*P4*P5,NULL,NULL,NULL,NULL,NULL,NULL,NULL',
	...
	select 
		value
	from string_split('P2*P4*P5,,P2*P4*P5,,P2*P4*P5,,,,,,,',',')

	select * from fc_filter_tmp_VNCORPVNWKS0850
	select * from fc_filter_tmp_Signature_VNCORPVNWKS0850
	select * from fc_filter_tmp_Selected_VNCORPVNWKS0850
	select * from FC_FUNCTION_ACTIVE_VNCORPVNWKS0850
*/
Alter Proc sp_Apply_Selected_function_into_system_New
	@Division				nvarchar(3),
	@FM_KEY					nvarchar(6),
	@List_ID				nvarchar(500),
	@List_Signature			nvarchar(500),
	@List_selected			nvarchar(500),
	@TypeView				nvarchar(20),--//''-->Gen, re-gen
...
BEGIN TRY
	...
	SELECT	
		...
		@n_err=0,
		@c_errmsg='', 
		@sp_name = 'sp_Apply_Selected_function_into_system_New',
		@USERS = SUSER_NAME(),
		@MODIFILED = GETDATE()
	...
	begin
		...
		select @sql=
		'Create table '+@table_filter_selected_tmp+'
		(
			[id]				int identity(1,1),
			[Selected]			bit default 0
		) '
	...
	begin
		...
		(
			...
			WHERE  TABLE_NAME='''+@tablename+'''
				AND COLUMN_NAME=''List_Signature''
		)
		begin
			ALTER TABLE '+@tablename+' ADD  [List_Signature] Nvarchar(500)
		end '
		if @debug>0
		begin
			select @sql 'add new column list signature'
		end
		execute(@sql)
		...
		(
			...
			FROM   INFORMATION_SCHEMA.COLUMNS
			WHERE  TABLE_NAME='''+@tablename+'''
				AND COLUMN_NAME=''Type_View''
		)
		begin
			ALTER TABLE '+@tablename+' ADD  [Type_View] Nvarchar(20)
		end '
		if @debug>0
		begin
			select @sql 'add new column Type_View'
		end
		execute(@sql)
		...
		' set 
			...
			[List_Signature]=replace(tmp.[List_Signature],''*'','',''),
			[Type_View]=tmp.[Type_View]
		from '+@tablename+' f
		Inner join
		(
			select 
				...
				[Selected],
				[Type_View]
			from '+@table_filter_tmp+' 
			where [selected]=1
		) tmp on tmp.[ID_Function]=f.[ID] 
