/*
	...
	exec sp_Promo_validate_mismatch 'PPD','202410',@b_Success OUT, @c_errmsg OUT

	select @b_Success b_Success, @c_errmsg c_errmsg

	/*
		MasterData-->select * from OT_Promo_validate_mismatch_MasterData
		--BOM_vs_masterbom-->select * from OT_Promo_validate_mismatch_BOM_vs_masterbom
		--BOM_vs_masterdata-->select * from OT_Promo_validate_mismatch_BOM_vs_masterdata
		RSP-->select * from OT_Promo_validate_mismatch_RSP
	*/
*/
Alter proc sp_Promo_validate_mismatch
	@Division		nvarchar(3),
	@FM_KEY			nvarchar(6),
...
BEGIN TRY
	...
	SELECT	
		...
		@n_err=0,
		@c_errmsg='', 
		@sp_name = 'sp_Promo_validate_mismatch',
		@USERS = SUSER_NAME(),
		@MODIFILED = GETDATE()
	...
	declare @FunctionName		nvarchar(50)='Promo'
	...
	begin
		...
		if @debug>0
		begin
			select 'OFFLINE PROMO SINGLE vs MasterData'
		end
		...
		(
			...
			from FC_SI_Promo_Single_'+@Division+@Monthfc+' f
			left join
			(
				select DISTINCT
					[SUB GROUP/ Brand],
		...
		if @debug>0
		begin
			select @sql '@sql OFFLINE PROMO SINGLE vs MasterData'
		end
		execute(@sql)
	...
	begin
		...
		if @debug>0
		begin
			select 'OFFLINE PROMO SINGLE vs RSP'
		end
		...
		if @debug>0
		begin
			select @sql+@sql1 '@sql OFFLINE PROMO SINGLE vs RSP'
		end
		execute(@sql+@sql1)
