/*
	...
	exec sp_FM_validate_mismatch 'PPD','202410',@b_Success OUT, @c_errmsg OUT
	...
	*/
*/
Alter proc sp_FM_validate_mismatch
	@Division		nvarchar(3),
	@FM_KEY			nvarchar(6),
...
BEGIN TRY
	...
	SELECT	
		...
		@n_err=0,
		@c_errmsg='', 
		@sp_name = 'sp_FM_validate_mismatch',
		@USERS = SUSER_NAME(),
		@MODIFILED = GETDATE()	
	...
	declare @FunctionName		nvarchar(50)='FM'
	...
	if @debug>0
	begin
		select 'FM OFFLINE SINGLE ONLY BASELINE vs MasterData'
	end	
	IF @n_continue=1
	begin
		...
		(
			select 
				...
				[EAN Code]=ISNULL(f.[EAN Code],''''),
				[SKU Code]=ISNULL(f.[SKU Code],''''),
		...
		if @debug>0
		begin
			select @sql '@sql OFFLINE SINGLE ONLY BASELINE vs MasterData'
		end
		execute(@sql)
	...
	if @debug>0
	begin
		select 'FM ONLINE SINGLE vs MasterData'
	end
	if @n_continue=1
	begin
		select @Type='MasterData'
		...
		(
			...
			where 
				...
				and ISNULL([Time series],'''')<>''Total Qty''
				and ISNULL([Item Category Group],'''')<>''LUMF''
				and ISNULL(s.[SUB GROUP/ Brand],'''')=''''
			group by
				s.[SUB GROUP/ Brand],
	...
	if @debug>0
	begin
		select 'FM ONLINE BOM vs masterbom'
	end
	if @n_continue=9
	begin
		...
		(
			select
				[Channel],
				[Time series],
				[EAN Code]=x1.[EAN Code],
				[SKU Code]=x1.[SKU Code],
				/*[Barcode Component]=zmr.Barcode_Component,
				[Material Component]=zmr.Material_Component,*/
				'+@listColumn_his_error_alert_set_0+','
				+@ListColumn_Current+'
			from
			(
				...
				where 
					s.[Item Category Group]=''LUMF''
				and ISNULL(f.[Local Level],'''')=''ONLINE''
				and ISNULL(f.[Time series],'''')<>''Total Qty''
				group by
					f.[Local Level],
					f.[Time series],
					f.[EAN Code],
			...
			) zmr on zmr.Material_Bom=x1.[SKU Code]
			where 
			(
				'+@ListColumn_Current_plus+'
			)<>0
			and ISNULL(zmr.[Material_Component],'''')=''''
		) as x '

		if @debug>0
		begin
			select @sql '@sql FM ONLINE BOM vs ZMR32'
		end
		execute(@sql)
	...
	if @debug>0
	begin
		select 'FM ONLINE Bom component vs Masterdata'
	end
	if @n_continue=9
	begin
		...
		(
			select
				[Channel],
				[Time series],
				[EAN Code]=zmr.Barcode_Component,
				[SKU Code]=zmr.Material_Component,
				'+@listColumn_his_error_alert_set_0+','
				+@ListColumn_Current+'
			from
			(
		...
		if @debug>0
		begin
			select @sql '@sql FM ONLINE Bom component vs Masterdata'
		end
		execute(@sql)
	...
	if @n_continue=1
	begin
		select @Division,@FM_KEY,@FunctionName
