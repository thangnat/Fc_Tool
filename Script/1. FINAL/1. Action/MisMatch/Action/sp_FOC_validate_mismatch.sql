/*
	...
	exec sp_FOC_validate_mismatch 'PPD','202410',@b_Success OUT, @c_errmsg OUT

	select @b_Success b_Success, @c_errmsg c_errmsg

	/*
		MasterData-->select * from validate_mismatch_MasterData
		--BOM_vs_masterbom-->select * from validate_mismatch_BOM_vs_masterbom
		--BOM_vs_masterdata-->select * from validate_mismatch_BOM_vs_masterdata
		RSP-->select * from validate_mismatch_RSP
	*/
*/
Alter proc sp_FOC_validate_mismatch
	@Division		nvarchar(3),
	@FM_KEY			nvarchar(6),
...
BEGIN TRY
	...
	SELECT	
		...
		@n_err=0,
		@c_errmsg='', 
		@sp_name = 'sp_FOC_validate_mismatch',
		@USERS = SUSER_NAME(),
		@MODIFILED = GETDATE()
	...
	if @debug>0
	begin
		select 'OFFLINE SINGLE FOC vs Masterdata'
	end
	if @n_continue=1
	begin
		...
		(
			...
			from FC_SI_FOC_SINGLE_'+@Division+@Monthfc+' f
			left join
			(
				select DISTINCT
					[SUB GROUP/ Brand],
	...
	if @debug>0
	begin
		select 'OFFLINE FOC SINGLE vs RSP'
	end
	if @n_continue=1
	begin
		select @Type='RSP'
		...
		if @debug>0
		begin
			select @sql+@sql1 '@sql OFFLINE FOC SINGLE vs RSP'
		end
		execute(@sql+@sql1)
