/*
	select 
		* 
	from V_FC_MMDOWNLOAD_SORTBY_PPD
	where Active = 1
...
Alter View V_FC_MMDOWNLOAD_SORTBY_PPD
As
select
	*
from
(
	Select 
		...
		[Active]
	from fnc_MM_ZMR54OLD_Stg('PPD') m
	left join 
	(
		select distinct
			[Active] = (select top 1 Active from FC_BFL_Master_PPD (NOLOCK) where [BFL Name] =s.[BFL Name] order by Active desc),
			[BFL Code],
			...
			[Franchise],
			[Sub Franchise],
			[Hero] = (select top 1 Hero from FC_BFL_Master_PPD (NOLOCK) where [BFL Name] =s.[BFL Name] AND ISNULL(Hero,'')<>'' order by Hero asc),
			[Product status] = isnull((select top 1 [Product Status] from FC_BFL_Master_PPD (NOLOCK) where [BFL Name] =s.[BFL Name] AND ISNULL([Product Status],'')<>'' order by [Product Status] asc),'')
		from FC_BFL_Master_PPD s (NOLOCK)	
