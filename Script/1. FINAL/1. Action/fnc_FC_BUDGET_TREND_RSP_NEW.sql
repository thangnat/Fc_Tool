/*
	select * 
	from fnc_FC_BUDGET_TREND_RSP_NEW('CPD','202502','B','B0')
	where [SUB GROUP/ Brand]='AGE REWIND CONCEALER'
*/

Alter function fnc_FC_BUDGET_TREND_RSP_NEW
(
	@division			nvarchar(3),
	@FM_KEY				nvarchar(6),
...
begin
	...
	begin
		...
		begin
			...
			(
				select 
					ID = ROW_NUMBER() over(partition by [SUB-GROUP],[channel] order by [SUB-GROUP] asc),
					[Filename],
					[FM_KEY],
					[Type],
					[Version],
