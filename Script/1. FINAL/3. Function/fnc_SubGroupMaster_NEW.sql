/*
	select * from fnc_SubGroupMaster_NEW('LLD','202412','full')
*/
Alter function fnc_SubGroupMaster_NEW
(
	@Division	Nvarchar(3),
	@FM_KEY		nvarchar(6),
	@Type		nvarchar(4)
...
As
begin
	declare @fm_key_ok nvarchar(6)=''

	if @FM_KEY=''
	begin
		select @fm_key_ok=(select top 1 FM_KEY from FC_ComputerName (NOLOCK) where [ComputerName]=HOST_NAME())
	end
	else
	begin
		select @fm_key_ok=@FM_KEY
	end
	...
	begin
		...
		(
			...
			where FM_KEY=case 
							when @FM_KEY='' then
								(select top 1 FM_KEY from FC_ComputerName (NOLOCK) where [ComputerName]=HOST_NAME())
							else
								@FM_KEY
						end
		) o on o.[SUB GROUP/ Brand] = s.[SUB GROUP/ Brand]
